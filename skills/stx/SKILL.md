...
- **Event hooks** — fire shell commands on any entity mutation (pre-hooks can veto; post-hooks are fire-and-forget). Config: `~/.config/stx/hooks.toml`. Inspect via `stx hook events|ls|validate|schema`. For authoring custom hooks interactively, use the nested `hooks` skill (`skills/stx/hooks/SKILL.md`). Full event catalog and recipe library in `references/hooks.md`.
