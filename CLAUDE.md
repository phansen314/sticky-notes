...
- **Entity metadata** — tasks, workspaces, projects, and groups each carry a JSON key/value blob (`metadata TEXT NOT NULL DEFAULT '{}' CHECK (json_valid(metadata))`). Keys are normalized to lowercase on write/read via `_normalize_meta_key()` in service (matching the codebase's `COLLATE NOCASE` convention, which doesn't apply to JSON fields directly). Key charset: `[a-z0-9_.-]+`, max 64 chars. Values are free-form text up to 500 chars. Generic helpers `_set_entity_meta` / `_get_entity_meta` / `_remove_entity_meta` take `setter` / `fetcher` / `remover` callables so the per-entity public functions (`set_task_meta`, `set_workspace_meta`, `set_project_meta`, `set_group_meta`, etc.) are one-line delegates. Repository mirrors this with generic `_set_metadata_key` / `_remove_metadata_key` helpers guarded by a `_METADATA_TABLES` allowlist. CLI CRUD via `todo {task,workspace,project,group} meta ls|get|set|del`; all commands return a uniform `{key, value}` shape in `--json` mode.
