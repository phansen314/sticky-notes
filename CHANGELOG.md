...
- **SQL statement splitter could slice mid-definition.** `connection.py`
  migration and schema loading no longer use `sql.split(";")`, which broke
  when a semicolon appeared inside a string literal or line comment. The
  new `_split_sql_statements` accumulates lines through
